...
_KEYWORD_QUERY_RULES = (
    ...
    "(8) cash-flow-statement metrics (capital expenditures, dividends paid, share "
    'repurchases, debt issued or repaid) live in rows that use transactional verbs '
    '("additions", "purchases", "payments for", "proceeds from", "repurchases"), not '
    'summary labels — match the noun the row shares ("property equipment", "dividends", '
    '"common stock") and never the MD&A summary term, which AND-excludes the row. '
    ...
    'capital expenditures / capex → "property equipment" (do NOT add "capital" or '
    '"expenditures" — those words appear only in MD&A prose, never in the cash-flow '
    'statement row that holds the figure, so AND-matching on them excludes the answer); '
...
1. SCOPE CHECK — A 10-K is the company's comprehensive annual report; it covers far more than the financial statements. It includes the business and operations, human capital and employees (headcount, unions, collective bargaining), properties, legal proceedings, risk factors, MD&A, the market for the stock and holders of record, related-party transactions, acquisitions and other arrangements, commitments, and subsequent events — alongside the audited financials and notes (which carry multiple years of comparatives, so year-over-year questions about one company are answerable).
   Mark as out_of_scope ONLY when the requested information falls outside what any SEC filing carries by design, namely: (a) market or trading data such as stock prices or market capitalisation; (b) third-party opinion such as analyst ratings or price targets; (c) genuinely forward-looking data the company does not itself publish, such as forward guidance or projections it never gave — this covers future-oriented statements only, not a historical or as-of reporting period; or (d) facts unrelated to the company. Anything a company discloses about its own business, people, operations, transactions, or finances is IN scope. When in doubt, proceed to retrieval — if the information is absent the downstream QA step will say so explicitly. DATE RECENCY IS NEVER GROUNDS FOR out_of_scope: you do not have a reliable sense of today's date, so never judge a fiscal year or reporting date to be "in the future" or too recent for a filing to exist. A specific past or as-of reporting date (e.g. "as of December 31, 2025") is a historical reporting period and is in scope. Whether a period's filing is actually present is decided by retrieval, never here — if it is absent, the downstream QA step will say so.
...
3. TICKER NORMALISATION — Identify the subject company (or companies) whose 10-K must be read to answer the query, and map each to its ticker symbol. Eg Apple → AAPL, Google/Alphabet → GOOGL, TSMC → TSM (use the US-listed symbol when available). Only the subject company needs a ticker — other entities named in the query (counterparties, partners, acquisition or arrangement targets) do NOT need to be mappable and are never grounds for out_of_scope; they are simply search terms inside the subject company's filing. Set query_type to "out_of_scope" only when the subject company itself cannot be confidently mapped to a known public ticker (e.g. the filing we would need belongs to a private company like OpenAI, or the company is too ambiguous to identify), and note the ambiguity in the reasoning field.
...
6. FISCAL YEAR — Extract the fiscal year the user is referring to as an integer (e.g. 2024). Do not attempt to resolve this to a calendar date — companies have non-calendar fiscal years. Leave null if no year is specified. If the user uses a relative reference such as "last year", "most recent", or "latest", also set fiscal_year to null and note the relative reference in resolved_query — do not attempt to guess a specific year. Never treat a stated fiscal year or as-of date as unretrievable or "in the future" based on your own sense of the current date — extract it normally and proceed; recency is never a reason to set query_type to out_of_scope.
...
5. DERIVATION CHECK — Are there any figures you would need to compute rather than quote directly? If so, mark them as unavailable — every figure marked here must be explicitly stated as not available in the answer. Note: do not confuse reading with computing — a figure that appears verbatim in the source (e.g. a table cell labelled "average price paid per share") is directly stated and may be quoted; you are reading that value, not performing arithmetic. Wrong-entity attribution also counts as derivation: if a chunk states a figure for entity A and you would attribute it to entity B (e.g., a consolidated margin attributed to a specific segment), that figure is not directly stated for entity B — mark it unavailable. Wrong-metric substitution also counts: a figure for a different metric that shares the asked metric's noun (e.g. gross PP&E balance or depreciation when capital expenditures was asked) is not the asked figure — mark it unavailable. Aggregation also counts: summing stated components into a total the source never states (e.g. adding two equity plans' share counts into a combined total, or two segments' revenues into a group revenue) is arithmetic — mark the total unavailable, report each component separately, and state that the source does not provide a combined figure. A query asking for a "total" or "combined" figure does not license you to compute one; only quote a total that appears verbatim in an excerpt.
...
- RANKING RULE — When the question asks for the largest, top-N, highest, or lowest entities by a metric, only entities whose value for that metric is explicitly stated in the context are eligible to be ranked. In a grouped table where a single figure sits on a parent-entity row followed by sub-rows (districts, locals, divisions, subsidiaries, sub-agreements) that carry no figure of their own, that figure belongs to the parent alone — rank the parent entity, never a sub-row, and never assign the parent's figure to a sub-row or count a sub-row as a separate entity.
- METRIC MATCH RULE — A figure is valid only if its row label, table header, or surrounding text names the SPECIFIC metric asked for — not merely the same noun. One noun maps to many distinct metrics: "property and equipment" appears as capital expenditures / additions / purchases (cash-flow statement), gross and net PP&E (balance sheet), depreciation (notes), and accrued unpaid capex (payables note) — these are NOT interchangeable. Before reporting a figure, confirm its label matches the asked metric. If the asked metric is not present under its own label, say the information is not available — never substitute a same-noun figure of a different metric. Direction also matters: revenue earned from, or cost of revenues on, a sale to a counterparty is not an amount paid to that counterparty — an amount received is never an amount paid, and vice versa. Do not report a figure whose direction is opposite to what the question asks; if the figure for the asked direction is not stated under its own label, say the information is not available.
...
- Be precise and thorough. Report the exact figures from the source material and all directly relevant supporting data. Include year-over-year comparisons, percentage changes, and explanations of what drove those changes only where the context explicitly states them. Do not stop at the headline number — if the context explains why a metric changed, include that explanation. When the context provides a breakdown of component figures for a composite metric, include those components — do not report only the aggregate. Conversely, when only the components are stated and the aggregate is not, report the components individually and do not sum them — state that the source does not provide a combined total rather than computing one.
...
4. DERIVATION CHECK — For each required figure: does it appear verbatim in an excerpt, or must it be computed? Do not confuse reading with computing — a figure that appears verbatim in the source (e.g. a table cell value) is directly stated and may be quoted; you are reading that value, not performing arithmetic. Any arithmetic, percentage calculation, or inference from multiple figures counts as a derivation — mark those as unavailable. Do not compute the derived value even to verify it — if a figure requires arithmetic, write "Not directly available" in the answer and stop; do not present the computed number. Wrong-entity attribution also counts as derivation: if a chunk states a figure for entity A (e.g., consolidated company operating margin) and you would attribute it to entity B (e.g., a specific segment), that figure is not directly stated for entity B — mark it "Not directly available". Wrong-metric substitution also counts as unavailable: if a chunk states a different metric that shares the asked metric's noun (e.g. gross PP&E balance or depreciation when capital expenditures was asked), that is not the asked figure — mark it "Not directly available". Aggregation also counts: summing stated components into a total the source never states (e.g. adding two equity plans' share counts into a combined total, or two segments' revenues into a group revenue) is arithmetic — write "Not directly available" for the total, report each component separately, and note the source provides no combined figure. A question asking for a "total" or "combined" figure does not license computing one; only quote a total that appears verbatim in an excerpt.
...
- ENTITY SCOPE RULE — Never use a consolidated or company-level figure (e.g., an operating margin from a company-wide income statement) as a segment, product-line, or region-level figure. Before citing any margin, ratio, or percentage, verify that the chunk's label or context explicitly scopes it to the exact entity the query asks about.
- RANKING RULE — When the question asks for the largest, top-N, highest, or lowest entities by a metric, only entities whose value for that metric is explicitly stated in the context are eligible to be ranked. In a grouped table where a single figure sits on a parent-entity row followed by sub-rows (districts, locals, divisions, subsidiaries, sub-agreements) that carry no figure of their own, that figure belongs to the parent alone — rank the parent entity, never a sub-row, and never assign the parent's figure to a sub-row or count a sub-row as a separate entity.
- METRIC MATCH RULE — A figure is valid only if its row label, table header, or surrounding text names the SPECIFIC metric asked for — not merely the same noun. One noun maps to many distinct metrics: "property and equipment" appears as capital expenditures / additions / purchases (cash-flow statement), gross and net PP&E (balance sheet), depreciation (notes), and accrued unpaid capex (payables note) — these are NOT interchangeable. Before reporting a figure, confirm its label matches the asked metric (capex = cash used for additions to / purchases of property and equipment). If the asked metric is not present under its own label, write "Not directly available" — never substitute a same-noun figure of a different metric. Direction also matters: revenue earned from, or cost of revenues on, a sale to a counterparty is not an amount paid to that counterparty — an amount received is never an amount paid, and vice versa. Do not report a figure whose direction is opposite to what the question asks; if the figure for the asked direction is not stated under its own label, write "Not directly available".
...
LOOP GUARD — A reformulation must change WHAT gets retrieved, not just edit tokens. Adding, dropping, or pluralising a word (e.g. "…equipment" → "…equipment total") does NOT count — it returns the same chunks. To count as a real change, switch the retrieval target: use different discriminative terms (e.g. move from a balance-sheet noun to the cash-flow verb the figure lives on — "additions"/"purchases"/"payments for"), change the section filter, or rephrase the semantic query to name a different statement. If no genuinely different reformulation is possible, set done: true.
...
1. SUFFICIENCY SCAN — One line per chunk: note chunk ID, what it covers, and whether it's sufficient. For each chunk you mark as sufficient for a specific metric, quote the exact phrase from the chunk text that states the figure (e.g. "chunk says: 'Additions to property and equipment: $64,551'"). If you cannot quote the figure verbatim from the chunk, the chunk is not sufficient for that metric. Metric-match: the quoted figure must be the SPECIFIC metric asked for, identified by its row or label — not merely a number sharing the metric's noun. For capital expenditures, "Additions to / Purchases of property and equipment" (cash-flow statement) counts; "gross property and equipment" (balance sheet), "depreciation expense", or "capex remaining in accounts payable" do NOT — treat those as a gap, not as sufficient. Note two special cases:
